...
```{python}
# My own version of code that determines the Hubble parameter at different redshifts
def H_zz(z,H0,Om,w0,w1):
    Ode=1-Om
    ...
    B1=np.array(b1)
    x=(Om*(1+z)**3) + Ode*np.exp(3*B1[:,0])
    Hzz=H0*np.sqrt(x)
    return Hzz
```

```{python}
# Astropy.cosmology version. Should be the same as above, but faster.
